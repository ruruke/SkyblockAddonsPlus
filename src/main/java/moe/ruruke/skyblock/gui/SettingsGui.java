...
public class SettingsGui extends GuiScreen {
    static final SkyblockAddonsPlus.Companion main = SkyblockAddonsPlus.Companion.getInstance();
    static final Logger logger = SkyblockAddonsPlus.Companion.getLogger();
    ...
    public void initGui() {
        ...
        if (feature == Feature.LANGUAGE) {
            Language currentLanguage = Language.Companion.getFromPath(main.getConfigValues().getLanguage().getPath());
...
//                DrawUtils.Companion.drawRect(x, y, width, height, ColorUtils.getDummySkyblockColor(28, 29, 41, 230), 4);
//            }
            SkyblockAddonsGui.Companion.drawScaledString(this, Translations.INSTANCE.getMessage("settings.settings"), 110, defaultBlue, 1.5, 0);
        }
    ...
    protected void actionPerformed(GuiButton abstractButton) {
        if (abstractButton instanceof ButtonLanguage) {
            ...
            DataUtils.INSTANCE.loadLocalizedStrings(language, false);
//            main.setKeyBindingDescriptions();
            ...
            Feature f = ((ButtonOpenColorMenu) abstractButton).getFeature();
    ...
    private void addButton(EnumUtils.FeatureSetting setting) {
        ...
        if (setting == EnumUtils.FeatureSetting.COLOR) {
            buttonList.add(new ButtonOpenColorMenu(x, y, 100, 20, Translations.INSTANCE.getMessage("settings.changeColor"), main, feature));
        } else if (setting == EnumUtils.FeatureSetting.GUI_SCALE) {
            try {
                buttonList.add(new ButtonGuiScale(x, y, 100, 20, main, feature));
            } catch (NumberFormatException e) {
                logger.error(e.getMessage());
                main.getUtils().sendMessage(Translations.INSTANCE.getMessage("messages.invalidFeatureConfiguration", feature.getMessage()));
                main.getConfigValues().setGuiScale(feature, main.getConfigValues().normalizeValueNoStep(1));
                buttonList.add(new ButtonGuiScale(x, y, 100, 20, main, feature));
            }
            ...
            buttonList.add(new ButtonToggleTitle(x, y, Translations.INSTANCE.getMessage("settings.repeating"), main, settingFeature));
            ...
            buttonList.add(new ButtonToggleTitle(x, y, Translations.INSTANCE.getMessage("settings.showInOtherGames"), main, settingFeature));
        } else if (setting == EnumUtils.FeatureSetting.BACKPACK_STYLE) {
            boxWidth = 140;
            x = halfWidth - (boxWidth / 2);
            buttonList.add(new ButtonSolid(x, y, 140, 20, Translations.INSTANCE.getMessage("settings.backpackStyle"), main, feature));
            ...
            buttonList.add(new ButtonToggleTitle(x, y, Translations.INSTANCE.getMessage("settings.enableMessageWhenActionPrevented"), main, settingFeature));
        } else if (setting == EnumUtils.FeatureSetting.POWER_ORB_DISPLAY_STYLE) {
            boxWidth = 140;
            x = halfWidth - (boxWidth / 2);
            buttonList.add(new ButtonSolid(x, y, 140, 20, Translations.INSTANCE.getMessage("settings.powerOrbDisplayStyle"), main, feature));
...
//            buttonList.add(new ButtonTextNew(halfWidth, (int) y - 10, setting == EnumUtils.FeatureSetting.DISCORD_RP_DETAILS ? Translations.INSTANCE.getMessage("messages.firstStatus") :
//                    Translations.INSTANCE.getMessage("messages.secondStatus"), true, 0xFFFFFFFF));
...
//                y = getRowHeightSetting(row);
//
//                buttonList.add(new ButtonTextNew(halfWidth, (int) y - 10, Translations.INSTANCE.getMessage("messages.fallbackStatus"), true, 0xFFFFFFFF));
...
//            }).setPrefix("Map Zoom: "));
            ...
            buttonList.add(new ButtonToggleTitle(x, y, Translations.INSTANCE.getMessage("settings.colorByRarity"), main, settingFeature));
            ...
            buttonList.add(new ButtonToggleTitle(x, y, Translations.INSTANCE.getMessage("settings.textMode"), main, settingFeature));
            ...
            row += .1;
            y = getRowHeightSetting(row);
            buttonList.add(new ButtonTextNew(halfWidth, (int) y + 15, Translations.INSTANCE.getMessage("settings.trevorTheTrapper.showQuestCooldownDescription"), true, ColorCode.GRAY.getColor()));
            row += 0.4;
            ...
            row += .4;
            y = getRowHeightSetting(row);
            buttonList.add(new ButtonTextNew(halfWidth, (int) y + 15, Translations.INSTANCE.getMessage("messages.entityOutlinesRequirement"), true, ColorCode.GRAY.getColor()));
            row += .4;
        } else {
            boxWidth = 31; // Default size and stuff.
            x = halfWidth - (boxWidth / 2);
