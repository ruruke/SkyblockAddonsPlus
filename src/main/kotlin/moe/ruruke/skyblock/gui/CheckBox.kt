...
class CheckBox internal constructor(
    mc: Minecraft, private val x: Int,
    private val y: Int, size: Int, text: String?
) {
    ...
    fun draw() {
        ...
        val color: Int = if (value) ColorCode.WHITE.getColor() else ColorCode.GRAY.getColor()
        DrawUtils.drawText(text, scaledX + Math.round(size * 1.5f / scale).toFloat(), scaledY + (size / 2f), color)
